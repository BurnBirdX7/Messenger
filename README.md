...
## Конструирование ПО
[\[В .docx\]](doc/Проектирование%20ПО.docx)

### Определение проблемы
Нынешние способы общения в интернете через мессенджеры или социальные сети не внушают нам доверия в плане анонимности
и защищенности данных.
Поэтому мы захотели разработать свой мессенджер с анонимностью, зашифрованностью и прозрачностью работы.


### Пользовательские истории
1.	Я хочу обмениваться текстовыми сообщениями в зашифрованном виде
2.	Я хочу сохранять анонимность при регистрации
3.	Я хочу, чтобы пароль от моей учетной записи был зашифрован
4.	Я хочу иметь возможность писать любому пользователю
5.	Я хочу иметь возможность находить пользователей по поиску логина
6.	Я хочу иметь возможность создавать и вступать в групповые чаты
7.	Я хочу иметь возможность при создании наложить на групповой чат пароль для вступления
8.	Я хочу, чтобы сохранялись мои чаты и их содержимое


### Диаграммы
**System Context Diagram**  
![System Context Diagram](doc/Context.png)  


**Container Diagram**  
![Container Diagram](doc/Container.png)
...
Программа написана на C++17 с использваонием следующих библиотек:
- [Boost](https://www.boost.org/) - большая кроссплатформенная библиотека для C++
  - Boost.Asio - библиотека для работы с сетью, с поддержкой SSL
  - Boost.PropertyTree - библиотека для парсинга конфигурационных файлов
- [OpenSSL](https://www.openssl.org/) - криптографическая библиотека, требуется для работы SSL в Boost.Asio
- [Google.Test](https://github.com/google/googletest) - библиотека Unit-тестов
- [PostgreSQL](https://www.postgresql.org/) 13 - СУБД сервера. <b>*</b>
- [pqxx](https://github.com/jtv/libpqxx) - высокоуровневая библиотека для работы с СУБД. <b>*</b>
- [Crypto++](https://www.cryptopp.com/) - криптографическая библиотека, требуется для шифрования паролей и данных сессий <b>*</b>

<b>*</b> - Требуется только для сборки сервера
...
(Например, в случае Windows это **wsock32**, **ws2_32**).
 ...
 - libpq будет найдена автоматически.

 - Для того чтобы подключить pqxx нужно задать переменную `PQXX_DIR`.  
Также нужно создать базу данных. Скрипт создания находится в корне репозитория: `db_init.sql`
...
Чтобы собрать только клиент: `cmake -build . --target Client`
...
Тесты собираются вместе с остальными компонентами.
Чтобы собрать их отдельно нужно выполнить `cmake --build . --target Google_Tests_run`.

## Комманды клиента

Приложение планировалось с графическим интерфейсом, но из-за недостатка времени в данный момент реализовано толлько консолькое взаимодействие.

|Комманда | Описание
|-------- | --------
|`login <username> <password>` | Войти в учётную запись
|`logoff` | Выйти из учётной записи
| `register <username> <password>` | Создать учётную запись
| `create <chatname> [password]` | Создать групповой чат. Чат должен обладать уникальным именем.
| `start <username>` | Создать личный чат
| `msg <chatname>` | Войти в чат и начать получать сообщения в реальном времени, получить возможность отправлять сообщения.
| `dmsg <username>` | Войти в личный чат, аналогично `msg`
| `quit` | Выйти из чата, возможно только после вызова `msg`/`dmsg`
| `leave <chatname>` | Перестать быть членом группового чата
